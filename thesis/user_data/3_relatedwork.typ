...
Diese Grenze zeigt sich bereits bei Toolformer. Das Verfahren beschränkt sich bei der Erzeugung und Nutzung von API-Aufrufen auf einzelne Aufrufe pro Beispiel. Dadurch kann es keine verketteten Toolaufrufe erlernen, bei denen beispielsweise die Ausgabe eines Werkzeugs als Input für ein weiteres Werkzeug dient. Auch eine interaktive Nutzung eines Werkzeugs, bei der beispielsweise eine Suchanfrage auf Grundlage zuvor erhaltener Ergebnisse angepasst wird, wird vom ursprünglichen Ansatz nicht unterstützt @schickToolformerLanguageModels2023.
...
Eine wichtige Unterscheidung für die Evaluation komplexerer Tool-Nutzung findet sich bei Lu et al. (2025) im Rahmen von OrchDAG. Dort werden Werkzeugausführungen als gerichtete azyklische Graphen (DAGs) modelliert. Ein Werkzeugaufruf entspricht hierbei einem Knoten. Abhängigkeiten zwischen Werzeugen werden als Kanten dargestellt. Ein Ausgabeparameter eines vorherigen Werkzeugaufrufs kann beispielsweise als Input für einen nachfolgenden Aufruf dienen. Dadurch lässt sich nicht nur bewerten, welche Werkzeuge verwendet werden, sondern auch, wie diese miteinander verbunden sind @luOrchDAGComplexTool2025.
